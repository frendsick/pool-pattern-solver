...
serves everything from cache, so the app needs no local or public network.
...
the explanation caption sits above it when the viewport is tall enough
...
- **Plain LAN HTTP**: simplest to serve, but not a secure context. There is no service worker,
