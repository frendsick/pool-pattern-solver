# Carom paths follow the 30-degree-rule shape without an engine
...
with the table. This is exact under the mirror-law abstraction.
...
- The locus walk prices candidate landings exactly before a rail. Curved routes
...
- `hitDistance` now lets follow use its top-spin roll share, but still prices
  ...
  less tangent speed (5/9). Recalibrating draw is a separate decision that is
  flagged but not taken.
