...
**Skill Profile**:
The parameter set describing the assumed player — P(pot) as a function of cut angle and distance, maximum makeable cut, position-speed error — used for both zone boundaries and scoring; v1 ships one fixed intermediate profile. P(pot) combines amplified aim error with object-ball direction noise at contact (throw, cloth roll-off) that is independent of cue-ball distance: the pocket's angular window shrinks with ball-to-pocket distance while that noise stays, so a long pot is punished even with the cue ball parked close behind — closer pockets require much less accuracy.
